    ...
    jint height)
{
  ...
  if (ThrowIfError(env, image_frame_or.status()))
    return 0L;
    ...
    JNIEnv *env, jobject thiz, jlong context, jobjectArray string_array)
{
  ...
  jsize count = env->GetArrayLength(string_array);

  // Create a vector to store the strings
  auto strings = absl::make_unique<std::vector<std::string>>();
  strings->reserve(count);

  // Convert each Java string to C++ string
  for (jsize i = 0; i < count; i++)
  {
    jstring j_string = (jstring)env->GetObjectArrayElement(string_array, i);
    if (j_string)
    {
      strings->push_back(mediapipe::android::JStringToStdString(env, j_string));
      env->DeleteLocalRef(j_string);
    }
    else
    {
      strings->push_back(""); // Handle null strings as empty strings
    }
  }

  // Create the packet
  mediapipe::Packet packet = mediapipe::Adopt(strings.release());
  return CreatePacketWithContext(context, packet);
}
    ...
    jint height, jobject texture_release_callback)
{
  ...
  if (ThrowIfError(env, buffer_or.status()))
    return 0L;
  mediapipe::Packet packet =
