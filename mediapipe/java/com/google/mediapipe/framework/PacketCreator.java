 ...
 *
 * <p>
 * This class provides a set of functions to create basic mediapipe packet
 * types.
 */
public class PacketCreator {
    ...
    /**
     * Create a MediaPipe Packet that contains a pointer to another MediaPipe
     * packet.
     *
     * <p>
     * This can be used as a way to update the value of a packet. Similar to a
     * mutable packet using mediapipe::AdoptAsUniquePtr.
     *
     * <p>
     * The parameter {@code packet} can be released after this call, since the
     * new packet already holds a reference to it in the native object.
     */
     ...
     *
     * <p>
     * Use {@link ByteBuffer#allocateDirect} when allocating the buffer. The
     * pixel rows should have 4-byte alignment.
     */
    ...
    /**
     * Create a MediaPipe audio packet that is used by most of the audio
     * calculators.
     *
     ...
     * calculators.
     *
     * @param data the raw audio data, bytes per sample is 2(only
     * AudioFormat.ENCODING_PCM_16BIT is supported). Must either be a direct
     * byte buffer or have an array, and the data has to be FILLED with
     * ByteOrder.LITTLE_ENDIAN byte order, which is ByteOrder.nativeOrder() on
     * Android (https://developer.android.com/ndk/guides/abis.html).
     * @param numChannels number of channels in the raw data.
     * @param numSamples number of samples in the data.
     */
    ...
    public Packet createStringArray(String[] value) {
        return Packet.create(nativeCreateStringArray(mediapipeGraph.getNativeHandle(), value));
    }
     ...
     *
     * <p>
     * Note: we are not populating frame rate and duration. If the calculator
     * needs those values, the calculator is not suitable here. Modify the
     * calculator to not require those values to work.
     */
    ...
    /**
     * Creates a mediapipe::TimeSeriesHeader, which is used by many audio
     * related calculators.
     *
    ...
    /**
     * Creates a {@link Packet} containing a {@code CalculatorOptions} proto
     * message.
     */
    ...
    /**
     * Creates a mediapipe::GpuBuffer with the specified texture name and
     * dimensions.
     *
     * @param name the OpenGL texture name.
     * @param width the width in pixels.
     * @param height the height in pixels.
     * @param releaseCallback a callback to be invoked when the
     * mediapipe::GpuBuffer is released. Can be null.
     */
     ...
     * @param height the height in pixels.
     * @deprecated use
     * {@link #createGpuBuffer(int,int,int,TextureReleaseCallback)} instead.
     */
    @Deprecated
     ...
     *
     * <p>
     * Note: in order for MediaPipe to be able to access the texture, the
     * application's GL context must be linked with MediaPipe's. This is ensured
     * by calling {@link
     * Graph#createGlRunner(String,long)} with the native handle to the
     * application's GL context as the second argument.
     */
    ...
    /**
     * Creates a 1, 3, or 4 channel 8-bit Image packet from a U8, RGB, or RGBA
     * byte buffer.
     *
     * <p>
     * Use {@link ByteBuffer#allocateDirect} when allocating the buffer.
    ...
    /**
     * Helper callback adaptor to create the Java {@link GlSyncToken}. This is
     * called by JNI code.
     */
    ...
    private native long nativeCreateStringArray(long context, String[] value);
